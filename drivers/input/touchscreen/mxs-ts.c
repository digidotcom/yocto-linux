...
#define TOUCH_DEBOUNCE_LAST_VALID	300
#define TOUCH_AVG_SAMPLES		5
...
static void process_lradc(struct mxs_ts_info *info, u16 x, u16 y,
			int pressure)
{
	static u16 lastx = TOUCH_MAX_X_COORD, lasty = TOUCH_MAX_X_COORD;
	...
	case TS_STATE_TOUCH_VERIFY:
		...
		if (0 == pressure) {
			/* When pressure value is 0, this is correct
			 * information in all displays.
			 */
			input_report_abs(info->idev, ABS_PRESSURE, 0);
			input_event(info->idev, EV_KEY, BTN_TOUCH, 0);
			input_sync(info->idev);
		}
		else {
			/* When pressure value is 1, in some displays
			 * may be correct information but in others
			 * the touch may be pressed or not.
			...
			if (info->x == TOUCH_MAX_X_COORD || info->y == TOUCH_MAX_Y_COORD) {
				...
				/* Skip samples separated too much from the last valid sample. This
				 * Workarounds problems with imprecise samples usually coming from a
				 * pen-up event */
				if (lastx == TOUCH_MAX_X_COORD || lasty == TOUCH_MAX_Y_COORD ||
				    ((abs(info->x - lastx) < TOUCH_DEBOUNCE_LAST_VALID) &&
				     (abs(info->y - lasty) < TOUCH_DEBOUNCE_LAST_VALID))) {
					input_report_abs(info->idev, ABS_X, info->x);
					input_report_abs(info->idev, ABS_Y, info->y);
					input_report_abs(info->idev, ABS_PRESSURE, 1);
					input_event(info->idev, EV_KEY, BTN_TOUCH, 1);
					input_sync(info->idev);
				}
			}
			lastx = info->x;
	...
	case TS_STATE_TOUCH_DETECT:
		...
		if (pressure) {
			...
			/* Prevent potential irresponsive status:
			 * In some ocasions, the TOUCH_DETECT status is entered
			 * with pressure = 0 and the pen lifted, but the status
			 * register shows the TOUCH_DETECT_RAW flag set. This
			 * leads to a situation where the touch lines don't
			 * have any voltage and a touch pressure is not capable
			 * of producing any interrupt. As a result, the touch
			 * does not respond at all. To prevent it, schedule a
			 * conversion.
			 */
			if (TS_STATE_TOUCH_DETECT == info->state &&
			    (__raw_readl(info->base + HW_LRADC_STATUS) &
