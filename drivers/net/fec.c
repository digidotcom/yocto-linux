...
#if defined(CONFIG_CCARDIMX28_ENET0_LEDS) || defined(CONFIG_CCARDIMX28_ENET1_LEDS)
static void toggle_activityled(struct net_device *dev)
{
	struct fec_enet_private *fep = netdev_priv(dev);
	...
	if(!timer_pending(&fep->activityled_timer))
		mod_timer(&fep->activityled_timer, jiffies + ACTIVITYLED_TOGGLE_TIMEOUT);
}

static void activityled_timer_fn(unsigned long dev)
{
	struct fec_enet_private *fep = netdev_priv((struct net_device *)dev);

	if(fep->rxtx_activity) {
		/* toggle RX/TX Ethernet activity LED */
		gpio_set_value(fep->activityled, !gpio_get_value(fep->activityled));
		mod_timer(&fep->activityled_timer, jiffies + ACTIVITYLED_TOGGLE_TIMEOUT);
		fep->rxtx_cnt = 0;
		fep->rxtx_activity = 0;
	} else {
		if(fep->rxtx_cnt++ < ACTIVITYLED_OFF_TIMEOUT / ACTIVITYLED_TOGGLE_TIMEOUT)
			mod_timer(&fep->activityled_timer, jiffies + ACTIVITYLED_TOGGLE_TIMEOUT);
		else {
			gpio_set_value(fep->activityled, 1); /* switch LED off  */
			fep->rxtx_cnt = 0;
		}
	}
...
#ifdef CONFIG_ARCH_MXS
	if (fep->num_tx) {
		/* Sometimes the DMA controller becomes confused and turns itself off even
		* though there's a descriptor that's ready in the ring.  If we see the
		* controller in this state and we have a descriptor in flight, give it a
		* kick so it can transmit the "stuck" descriptor. */
		writel(0, fep->hwp + FEC_X_DES_ACTIVE);
	}
#endif
	...
	while (!((status = bdp->cbd_sc) & BD_ENET_RX_EMPTY)) {
		...
		if (status & (BD_ENET_RX_LG | BD_ENET_RX_SH | BD_ENET_RX_NO |
			...
			if (status & BD_ENET_RX_CR)	/* CRC Error */
				 ...
				 * Most packets on eth1 and some on eth0 are marked with CRC errors
				 * while they seem to be correct. Will be clarified in the next
...
#if  defined(CONFIG_CCARDIMX28_ENET0_LEDS) || defined(CONFIG_CCARDIMX28_ENET1_LEDS)
	gpio_set_value(fep->linkled, !fep->link);
#endif
...
static struct mii_bus *fec_enet_mii_init(struct platform_device *pdev)
{
	...
	/*
	 * Set MII speed to 2.5 MHz (= clk / 2 * phy_speed)
	 * clk differs between MXC and MXS families
	 * For MXS, it is the HCLK.
	 * For MXC, it is the ENET_CLK.
	 */
#ifdef CONFIG_ARCH_MXS
	pclk = clk_get(NULL, "h");
#else
	 ...
	 * holdtime = PCLK/100MHz. No need to round up as 0 equates
	 * to 1 pclk cycle, 1 to 2 cycles, and so on.
	 * (PCLK is HBUS clock on MXS family)
	 */
	fep->phy_holdtime = BF_ENET_MSCR_HOLDTIME(clk_get_rate(pclk)/100000000);
#endif
	...
	mscr = fep->phy_speed;
#ifdef CONFIG_ARCH_MXS
	mscr |= fep->phy_holdtime;
#endif
	writel(mscr, fep->hwp + FEC_MII_SPEED);
...
fec_probe(struct platform_device *pdev)
{
	...
	fep->activityled = pdata->activityled;
	fep->linkled = pdata->linkled;
	fep->rxtx_cnt = 0;
	fep->activityled_timer.data = (unsigned long)ndev;
	fep->activityled_timer.function = activityled_timer_fn;
	fep->activityled_timer.expires = jiffies + ACTIVITYLED_TOGGLE_TIMEOUT;
...
#if defined(CONFIG_CCARDIMX28_ENET0_LEDS) || defined(CONFIG_CCARDIMX28_ENET1_LEDS)
	del_timer(&fep->activityled_timer);
#endif
