...
#ifdef CONFIG_SERIAL_MXS_GPIO_CTS_RTS
	int gpio_cts_pin;
	int gpio_rts_pin;
#endif
};
...
static void mxs_auart_set_mctrl(struct uart_port *u, unsigned mctrl)
{
	...
	{
		if (mctrl & TIOCM_RTS)
			gpio_set_value(s->gpio_rts_pin, 0);
		else
			gpio_set_value(s->gpio_rts_pin, 1);
	}
	else
#endif
	{
...
static u32 mxs_auart_get_mctrl(struct uart_port *u)
{
	...
	if (s->gpio_cts_pin >= 0)
	{
		if (gpio_get_value(s->gpio_cts_pin) == 0)
			mctrl |= TIOCM_CTS;
	}
	else
#endif
	{
		...
		int ctrl2 = readl(u->membase + HW_UARTAPP_CTRL2);
	...
	/*
	* Unsupported signals: DCD/DSR permanently active, RI permanently
	* inactive.
	*/
...
#ifdef CONFIG_SERIAL_MXS_GPIO_CTS_RTS
static irqreturn_t mxs_auart_irq_cts(int irq, void *context)
{
	struct mxs_auart_port *s = context;
	unsigned int status;

	status = mxs_auart_get_mctrl(&s->port);
	set_irq_type(irq, (status & TIOCM_CTS) ? IRQ_TYPE_EDGE_RISING
...
#ifdef CONFIG_SERIAL_MXS_GPIO_CTS_RTS
	if (s->gpio_cts_pin >= 0)
		free_irq(gpio_to_irq(s->gpio_cts_pin), s);
#endif
...
#ifdef CONFIG_SERIAL_MXS_GPIO_CTS_RTS
	if (s->gpio_cts_pin >= 0) {
		err = request_irq(gpio_to_irq(s->gpio_cts_pin),
		    		  mxs_auart_irq_cts,
		    		  IRQF_TRIGGER_LOW,
		    		  "auart cts", s);
		if (err)
...
{
	struct mxs_auart_port *s = to_auart_port(u);
	unsigned int reg, databits, stopbits = 1, totalbits;

	/* Do a delay to make sure the shift register is
	 * emptied before closing the port. The delay is
	 * calculated for transmitting 1 start bit +
	 * set databits + set stop bits + 1 parity bit
	 */
	reg = __raw_readl(u->membase + HW_UARTAPP_LINECTRL);
	databits = 5 + ((reg & BM_UARTAPP_LINECTRL_WLEN) >> BP_UARTAPP_LINECTRL_WLEN);
	if (reg & BM_UARTAPP_LINECTRL_STP2)
		stopbits = 2;
	totalbits = databits + stopbits + 1 + 1;
	udelay(totalbits * 1000000 / s->baudrate);
...
static int __init auart_console_setup(struct console *co, char *options)
{
	...
	port = to_auart_port(&auart_port[co->index].port);
	...
	if (port->port.membase == 0) {
		if (cpu_is_mx23()) {
            ...
            switch (co->index) {
               case 0:
                       port->port.membase = IO_ADDRESS(0x8006A000);
                       port->port.mapbase = 0x8006A000;
                       break;
               case 1:
                       port->port.membase = IO_ADDRESS(0x8006C000);
                       port->port.mapbase = 0x8006C000;
               ...
               case 3:
                       ...
                       break;
               case 4:
                       port->port.membase = IO_ADDRESS(0x80072000);
                       port->port.mapbase = 0x80072000;
                       break;
               }
        }
	...
	mxs_auart_reset(&auart_port[co->index].port);
	...
	if (options)
		...
		auart_console_get_options(&auart_port[co->index].port, &baud, &parity, &bits);

	return uart_set_options(&auart_port[co->index].port, co, baud, parity, bits, flow);
}
...
#ifdef CONFIG_SERIAL_MXS_GPIO_CTS_RTS
	s->gpio_cts_pin = plat->gpio_cts_pin;
	s->gpio_rts_pin = plat->gpio_rts_pin;
#endif
