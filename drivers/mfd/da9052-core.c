...
static int add_da9052_devices(struct da9052 *da9052)
{
	...
	struct da9052_backlight_data backlight_data = *(pdata->backlight_data);
#endif
	...
	ret = da9052_add_subdevice_pdata(da9052, "WLED-4",
	...
	ret = da9052_add_subdevice_pdata(da9052, "WLED-5",
				&backlight_data, sizeof(backlight_data));
	if (ret)
		return ret;
#endif
...
int da9052_ssc_init(struct da9052 *da9052)
{
	...
	} else if ((ssc_msg.data & DA9052_CHIPID_MRC) == 0xb0) {
		da9052->chip_version = DA9053_VERSION_CC;
		pr_info("CC version probed\n");
	} else {
