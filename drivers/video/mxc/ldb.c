...
#if defined (CONFIG_JSCCIMX53_V2) || defined(CONFIG_JSCCIMX53_V1)
					if (ipu_di == i)
#endif
...
static int ldb_fb_pre_setup(struct fb_info *fbi)
{
	...
	/* If splitting, divide by 2 the parent clock freq, because there are 2 video streams */
	if (ldb.chan_mode_opt == LDB_SPL_DI0 || ldb.chan_mode_opt == LDB_SPL_DI1)
		ldb_clk_prate /= 2;
...
static int ldb_probe(struct platform_device *pdev)
{
	...
	if (g_di0_used) {
		gpio_lvds_active(0);
		/* If split or dual, configure also LVDS1 iomux for lvds operation */
		if (g_chan_mode_opt == LDB_SPL_DI0 || g_chan_mode_opt == LDB_DUL_DI0)
			gpio_lvds_active(1);
	...
	if (g_di1_used) {
		gpio_lvds_active(1);
		/* If split or dual, configure also LVDS0 iomux for lvds operation */
		if (g_chan_mode_opt == LDB_SPL_DI1 || g_chan_mode_opt == LDB_DUL_DI1)
			gpio_lvds_active(0);
