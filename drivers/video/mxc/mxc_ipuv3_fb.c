...
extern void ipu_debugfs_snapshot( void );
#endif
...
static int mxcfb_setup(struct fb_info *fbi, struct platform_device *pdev)
{
...
#ifdef CONFIG_CCIMX5X_DEFAULT_VIDEO_BPP
		mxcfbi->default_bpp = CONFIG_CCIMX5X_DEFAULT_VIDEO_BPP;
#else
	...
	fbi->var.activate |= FB_ACTIVATE_FORCE | FB_ACTIVATE_ALL;
	acquire_console_sem();
