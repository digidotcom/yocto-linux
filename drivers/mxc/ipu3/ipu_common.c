...
static struct debugfs_blob_wrapper ipu_debug_blob;
static struct dentry *ipu_debugfs_dump_dentry;
static struct dentry *ipu_debugfs_folder;
static char * g_ipu_debugfs_data;
#endif
...
#ifdef CONFIG_DEBUG_FS
static struct dentry *ipu_debugfs_dump(const char *name,
		struct dentry * folder,
		struct debugfs_blob_wrapper * blob)
{
	int di,i;

	g_ipu_debugfs_data = kzalloc(300 * MAX_LINE_LENGTH, GFP_KERNEL);
	if (!g_ipu_debugfs_data)
		return NULL;

	blob->data = g_ipu_debugfs_data;
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_CONF = \t0x%08X\n", __raw_readl(IPU_CONF));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "SMFC_MAP = \t0x%08X\n", __raw_readl(SMFC_MAP));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "SMFC_WMC = \t0x%08X\n", __raw_readl(SMFC_WMC));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_CONF = \t0x%08X\n", __raw_readl(IDMAC_CONF));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_CHA_EN1 = \t0x%08X\n", __raw_readl(IDMAC_CHA_EN(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_CHA_EN2 = \t0x%08X\n", __raw_readl(IDMAC_CHA_EN(32)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_CHA_PRI1 = \t0x%08X\n", __raw_readl(IDMAC_CHA_PRI(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_CHA_PRI2 = \t0x%08X\n", __raw_readl(IDMAC_CHA_PRI(32)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_BAND_EN1 = \t0x%08X\n", __raw_readl(IDMAC_BAND_EN(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IDMAC_BAND_EN2 = \t0x%08X\n", __raw_readl(IDMAC_BAND_EN(32)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_CHA_DB_MODE_SEL0 = \t0x%08X\n", __raw_readl(IPU_CHA_DB_MODE_SEL(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_CHA_DB_MODE_SEL1 = \t0x%08X\n", __raw_readl(IPU_CHA_DB_MODE_SEL(32)));
	if (g_ipu_hw_rev >= 2) {
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_CHA_TRB_MODE_SEL0 = \t0x%08X\n", __raw_readl(IPU_CHA_TRB_MODE_SEL(0)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_CHA_TRB_MODE_SEL1 = \t0x%08X\n", __raw_readl(IPU_CHA_TRB_MODE_SEL(32)));
	}

	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DMFC_WR_CHAN = \t0x%08X\n", __raw_readl(DMFC_WR_CHAN));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DMFC_WR_CHAN_DEF = \t0x%08X\n", __raw_readl(DMFC_WR_CHAN_DEF));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DMFC_DP_CHAN = \t0x%08X\n", __raw_readl(DMFC_DP_CHAN));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DMFC_DP_CHAN_DEF = \t0x%08X\n", __raw_readl(DMFC_DP_CHAN_DEF));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DMFC_IC_CTRL = \t0x%08X\n", __raw_readl(DMFC_IC_CTRL));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_FS_PROC_FLOW1 = \t0x%08X\n", __raw_readl(IPU_FS_PROC_FLOW1));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_FS_PROC_FLOW2 = \t0x%08X\n", __raw_readl(IPU_FS_PROC_FLOW2));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_FS_PROC_FLOW3 = \t0x%08X\n", __raw_readl(IPU_FS_PROC_FLOW3));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_FS_DISP_FLOW1 = \t0x%08X\n", __raw_readl(IPU_FS_DISP_FLOW1));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_CTRL_1 = \t0x%08X\n", __raw_readl(IPU_INT_CTRL (1)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_1 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (1)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_2 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (2)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_3 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (3)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_4 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (4)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_5 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (5)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IPU_INT_STAT_6 = \t0x%08X\n", __raw_readl(IPU_INT_STAT (6)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI0_SENS_CONF = \t0x%08X\n", __raw_readl(CSI_SENS_CONF (0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI0_SENS_FRM_SIZE = \t0x%08X\n", __raw_readl(CSI_SENS_FRM_SIZE(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI0_ACT_FRM_SIZE = \t0x%08X\n", __raw_readl(CSI_ACT_FRM_SIZE(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI0_SKIP = \t0x%08X\n", __raw_readl(CSI_SKIP(0)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI1_SENS_CONF = \t0x%08X\n", __raw_readl(CSI_SENS_CONF (1)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI1_SENS_FRM_SIZE = \t0x%08X\n", __raw_readl(CSI_SENS_FRM_SIZE(1)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI1_ACT_FRM_SIZE = \t0x%08X\n", __raw_readl(CSI_ACT_FRM_SIZE(1)));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "CSI1_SKIP = \t0x%08X\n", __raw_readl(CSI_SKIP(1)));

	for(di=0 ; di < 2 ; di++){
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "---- Display %d ----\n",di);
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_GENERAL = \t0x%08X\n", __raw_readl(DI_GENERAL(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_BS_CLKGEN0 = \t0x%08X\n", __raw_readl(DI_BS_CLKGEN0(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_BS_CLKGEN1 = \t0x%08X\n", __raw_readl(DI_BS_CLKGEN1(di)));


		for(i=1; i<=9; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_SW_GEN0(di,i)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_SW_GEN1: ");
		for(i=1; i<=9; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_SW_GEN1(di,i)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_SYNC_AS_GEN = \t0x%08X\n", __raw_readl(DI_SYNC_AS_GEN(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_DW_GEN: ");
		for(i=0; i<=11; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_DW_GEN(di,i)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_DW_SET0: ");
		for(i=0; i<=11; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_DW_SET(di,i,0)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_DW_SET1: ");
		for(i=0; i<=11; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_DW_SET(di,i,1)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_DW_SET2: ");
		for(i=0; i<=11; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_DW_SET(di,i,2)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_DW_SET3: ");
		for(i=0; i<=11; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_DW_SET(di,i,3)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_STP_REP: ");
		for(i=0; i<=3; i++)
			g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "[%d] \t0x%08X\n",i, __raw_readl(DI_STP_REP(di,i)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_SER_CONF = \t0x%08X\n", __raw_readl(DI_SER_CONF(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_SSC = \t0x%08X\n", __raw_readl(DI_SSC(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_POL = \t0x%08X\n", __raw_readl(DI_POL(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_AW0 = \t0x%08X\n", __raw_readl(DI_AW0(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_AW1 = \t0x%08X\n", __raw_readl(DI_AW1(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_SCR_CONF = \t0x%08X\n", __raw_readl(DI_SCR_CONF(di)));
		g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "DI_STAT = \t0x%08X\n", __raw_readl(DI_STAT(di)));
	}

	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_CONF = \t0x%08X\n",__raw_readl(IC_CONF));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_PRP_ENC_RSC = \t0x%08X\n",__raw_readl(IC_PRP_ENC_RSC));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_PRP_VF_RSC = \t0x%08X\n",__raw_readl(IC_PRP_VF_RSC));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_PP_RSC = \t0x%08X\n",__raw_readl(IC_PP_RSC));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_IDMAC_1 = \t0x%08X\n",__raw_readl(IC_IDMAC_1));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_IDMAC_2 = \t0x%08X\n",__raw_readl(IC_IDMAC_2));
	g_ipu_debugfs_data += sprintf(g_ipu_debugfs_data, "IC_IDMAC_3 = \t0x%08X\n",__raw_readl(IC_IDMAC_3));

	blob->size = strlen(blob->data);
	return debugfs_create_blob(name, S_IRUSR, ipu_debugfs_folder, blob);
}

static void ipu_debugfs_init( void )
{
	ipu_debugfs_folder = debugfs_create_dir("ipu", NULL);
	if (IS_ERR(ipu_debugfs_folder) || !ipu_debugfs_folder){
		printk(KERN_WARNING "Unable to create debugfs folder, is debugfs"
				"enabled?\n");
		return;
	}
...
void ipu_debugfs_snapshot( void )
{
	if( ipu_debugfs_dump_dentry )
		debugfs_remove(ipu_debugfs_dump_dentry);
	ipu_debugfs_dump_dentry =
			ipu_debugfs_dump("dump", ipu_debugfs_folder , &ipu_debug_blob );
	if (IS_ERR(ipu_debugfs_dump_dentry) || !ipu_debugfs_dump_dentry)
		printk(KERN_WARNING "Unable to create debugfs dump file.\n");
	return;
}

static void ipu_debugfs_remove( void )
{
	debugfs_remove_recursive(ipu_debugfs_folder);
	if(g_ipu_debugfs_data)
		kfree(g_ipu_debugfs_data);
}
#endif
...
static int ipu_probe(struct platform_device *pdev)
{
	...
	if (g_ipu_hw_rev == 3)
		__raw_writel(0x002A0000, IDMAC_CH_LOCK_EN_1);
...
int ipu_is_init_channel(ipu_channel_t channel)
{
...
EXPORT_SYMBOL(ipu_is_rgb);
#endif

char * pixelformat_str( int pixel_fmt )
{
	switch (pixel_fmt){
		case IPU_PIX_FMT_RGB666:
...
static int ipu_channel_from_id( int chan_id )
{
	switch(chan_id){
	case 1:
		return MEM_ROT_ENC_MEM;
	case 2:
		return MEM_ROT_VF_MEM;
	case 3:
		return MEM_ROT_PP_MEM;
	case 4:
		return MEM_PRP_ENC_MEM;
	case 5:
		return MEM_PRP_VF_MEM;
	case 6:
		return MEM_PP_MEM;
	case 7:
		return MEM_DC_SYNC;
	...
	case 10:
		return MEM_FG_SYNC;
	case 11:
		return MEM_BG_ASYNC0;
	case 12:
		return MEM_FG_ASYNC0;
	case 13:
		return DIRECT_ASYNC0;
	case 14:
		return DIRECT_ASYNC1;
	case 15:
		return CSI_MEM0;
	case 16:
		return CSI_MEM1;
	...
	case 18:
		return CSI_MEM3;
	case 19:
		return CSI_PRP_ENC_MEM;
	case 20:
		return CSI_PRP_VF_MEM;
	case 21:
		return MEM_VDI_PRP_VF_MEM_P;
	case 22:
		return MEM_VDI_PRP_VF_MEM;
	case 23:
		return MEM_VDI_PRP_VF_MEM_N;
	default:
		return CHAN_NONE;
	}
}
...
static int ipu_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	/* Digi CCORE_MX53_EXTENSIONS-126 */
#ifndef CONFIG_MXC_IPU_V1
	if (g_channel_init_mask) {
		for( ipuchan = 0 ; ipuchan < 32 ; ipuchan++ ){
			if(g_channel_init_mask & (1 << ipuchan)){
				ipu_disable_channel(ipu_channel_from_id(ipuchan),true);
				ipu_uninit_channel(ipu_channel_from_id(ipuchan));
			}
		}
	}
#endif
