		...
		int max_iterations, u32 exp_val)
{
	...
	for (i = 0; i < max_iterations + 1; i++) {
		...
		schedule_timeout_uninterruptible(usecs_to_jiffies(2000));
	}
	return 0;
}
...
#define is_ccimx53()	(machine_is_ccimx53js() || machine_is_ccimx53() )
...
static int sata_init(struct device *dev)
{
	...
	if (machine_is_mx53_smd() || machine_is_mx53_loco() || is_ccimx53()
			|| board_is_mx53_ard_b()) {
