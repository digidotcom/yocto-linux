...
static int mx28_usb_clks_enable(struct clk *clk)
{
	unsigned int reg;
	if (clk->enable_reg) {
		reg = __raw_readl(clk->enable_reg);
...
static void mx28_usb_clks_disable(struct clk *clk)
{
	unsigned int reg;
	if (clk->enable_reg) {
		reg = __raw_readl(clk->enable_reg);
...
{
	u32 clkctrl_dis_lcdif;
	u32 div;

	pr_debug("%s: rate %lu, parent rate %lu\n", __func__, rate,
		 clk_get_rate(clk->parent));

	/* Clock gated off to change frequencies */
	clkctrl_dis_lcdif = __raw_readl(clk->scale_reg);
	clkctrl_dis_lcdif |= BM_CLKCTRL_DIS_LCDIF_CLKGATE;
	__raw_writel(clkctrl_dis_lcdif, clk->scale_reg);
	div = (clk_get_rate(clk->parent) + rate - 1) / rate;

	pr_debug("%s: div calculated is %d\n", __func__, div);

	if (div == 0 || div > BM_CLKCTRL_DIS_LCDIF_DIV)
		return -EINVAL;

	clkctrl_dis_lcdif &= ~(BM_CLKCTRL_DIS_LCDIF_DIV_FRAC_EN | BM_CLKCTRL_DIS_LCDIF_DIV);
	clkctrl_dis_lcdif &= ~BM_CLKCTRL_DIS_LCDIF_CLKGATE;
	clkctrl_dis_lcdif |= (div & BM_CLKCTRL_DIS_LCDIF_DIV);
	__raw_writel(clkctrl_dis_lcdif, clk->scale_reg);
	...
	if (clk->busy_reg) {
		...
		for (i = 10000; i; i--)
			if (!clk_is_busy(clk))
				break;
		if (!i) {
			pr_err("couldn't set up DIS_LCDIF clk divisor\n");
			return -ETIMEDOUT;
		}
	}
...
static struct clk usb_phy_clk0 = {
	.parent = &pll_clk[0],
	.enable = mx28_usb_clks_enable, /* EN_USB_CLKS = 1 means ON */
	.disable = mx28_usb_clks_disable,
