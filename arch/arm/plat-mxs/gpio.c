...
static int mxs_gpio_wakeup_configure(struct gpio_chip *chip, unsigned int index, int value)
{
	struct mxs_gpio_port *port;
	int result = -1;
	...
	if (port->chip->wakeup_configure) {
		port->chip->wakeup_configure(port, index, value);
		result = 0;
	}
...
void mxs_set_gpio_wake(int (*set_wake)(unsigned int, unsigned int))
{
	gpio_irq_chip.set_wake = set_wake;
}
...
int __init mxs_add_gpio_port(struct mxs_gpio_port *port)
{
	...
	if (port->chip->wakeup_configure) {
		port->port.wakeup_configure = mxs_gpio_wakeup_configure;
	}
