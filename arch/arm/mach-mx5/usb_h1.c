...
static int gpio_usbh1_active(void)
{
	...
	if( machine_is_ccimx53js() ){
		...
		gpio_set_value(CCIMX53_USB_HUB_RESET, 0);
		msleep(1);
		gpio_set_value(CCIMX53_USB_HUB_RESET, 1);
	}
...
static void gpio_usbh1_inactive(void)
{
	...
	if( machine_is_ccimx53js() || machine_is_ccmx53js() ||
		machine_is_ccimx53js() ){
		gpio_free(CCIMX53_USB_HUB_RESET);
	}
}
