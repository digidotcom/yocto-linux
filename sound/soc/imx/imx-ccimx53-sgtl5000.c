/*
 * imx-ccimx53-sgtl5000.c  --  i.MX CCIMX53 Driver for Freescale SGTL5000 Codec
 *
...
struct imx_ccimx53_priv {
	int sysclk;
	int hw;
	struct platform_device *pdev;
};

static struct imx_ccimx53_priv card_priv;

static int imx_ccimx53_audio_hw_params(struct snd_pcm_substream *substream,
				      struct snd_pcm_hw_params *params)
{
	...
	struct imx_ccimx53_priv *priv = &card_priv;
...
static int imx_ccimx53_startup(struct snd_pcm_substream *substream)
{
#if defined(CONFIG_MXC_ASRC) || defined(CONFIG_MXC_ASRC_MODULE)
...
static void imx_ccimx53_shutdown(struct snd_pcm_substream *substream)
{
	struct imx_ccimx53_priv *priv = &card_priv;
...
/*
 * imx_ccimx53 SGTL5000 audio DAI opserations.
 */
static struct snd_soc_ops imx_ccimx53_ops = {
	.startup = imx_ccimx53_startup,
	.shutdown = imx_ccimx53_shutdown,
	.hw_params = imx_ccimx53_audio_hw_params,
};

static void imx_ccimx53_init_dam(int ssi_port, int dai_port)
{
...
/* imx_ccimx53 machine connections to the codec pins */
...
/* imx_ccimx53 card dapm widgets */
static const struct snd_soc_dapm_widget imx_ccimx53_dapm_widgets[] = {
...
static int imx_ccimx53_sgtl5000_init(struct snd_soc_codec *codec)
{
	int i, ret;
	...
	/* Add imx_ccimx53 specific controls */
	...
	/* Add imx_ccimx53 specific widgets */
	snd_soc_dapm_new_controls(codec, imx_ccimx53_dapm_widgets,
				  ARRAY_SIZE(imx_ccimx53_dapm_widgets));

	/* Set up imx_ccimx53 specific audio path audio_map */
...
/* imx_ccimx53 digital audio interface glue - connects codec <--> CPU */
static struct snd_soc_dai_link imx_ccimx53_dai = {
	.name = "SGTL5000",
	.stream_name = "SGTL5000",
	.codec_dai = &sgtl5000_dai,
	.init = imx_ccimx53_sgtl5000_init,
	.ops = &imx_ccimx53_ops,
};

static int imx_ccimx53_card_remove(struct platform_device *pdev)
{
	struct imx_ccimx53_priv *priv = &card_priv;
	struct mxc_audio_platform_data *plat;
	if (priv->pdev) {
...
static struct snd_soc_card snd_soc_card_imx_ccimx53 = {
	.name = "imx-ccimx53",
	.platform = &imx_soc_platform,
	.dai_link = &imx_ccimx53_dai,
	.num_links = 1,
	.remove = imx_ccimx53_card_remove,
};

static struct snd_soc_device imx_ccimx53_snd_devdata = {
	.card = &snd_soc_card_imx_ccimx53,
	.codec_dev = &soc_codec_dev_sgtl5000,
};

static int __devinit imx_ccimx53_sgtl5000_probe(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_ccimx53_priv *priv = &card_priv;
	...
	imx_ccimx53_init_dam(plat->src_port, plat->ext_port);
	...
	imx_ccimx53_dai.cpu_dai = sgtl5000_cpu_dai;

	/* get mxc_audio_platform_data for pcm */
	imx_ccimx53_dai.cpu_dai->dev = &pdev->dev;
	...
	imx_ccimx53_snd_devdata.codec_data = setup;
...
static int imx_ccimx53_sgtl5000_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_ccimx53_priv *priv = &card_priv;
...
static struct platform_driver imx_ccimx53_sgtl5000_audio_driver = {
	.probe = imx_ccimx53_sgtl5000_probe,
	.remove = imx_ccimx53_sgtl5000_remove,
	.driver = {
		   .name = "imx-ccimx53-sgtl5000",
		   },
};

static struct platform_device *imx_ccimx53_snd_device;

static int __init imx_ccimx53_init(void)
{
	int ret;
	ret = platform_driver_register(&imx_ccimx53_sgtl5000_audio_driver);
	if (ret)
		return -ENOMEM;

	imx_ccimx53_snd_device = platform_device_alloc("soc-audio", 2);
	if (!imx_ccimx53_snd_device)
		return -ENOMEM;

	platform_set_drvdata(imx_ccimx53_snd_device, &imx_ccimx53_snd_devdata);
	imx_ccimx53_snd_devdata.dev = &imx_ccimx53_snd_device->dev;
	ret = platform_device_add(imx_ccimx53_snd_device);

	if (ret)
		platform_device_put(imx_ccimx53_snd_device);

	return ret;
}

static void __exit imx_ccimx53_exit(void)
{
	platform_driver_unregister(&imx_ccimx53_sgtl5000_audio_driver);
	platform_device_unregister(imx_ccimx53_snd_device);
}

module_init(imx_ccimx53_init);
module_exit(imx_ccimx53_exit);

MODULE_AUTHOR("Digi International.");
MODULE_DESCRIPTION("SGTL5000 Driver for CCIMX53.");
MODULE_LICENSE("GPL");
