...
static void ieee80211softmac_associated(struct ieee80211softmac_device *mac,
	struct ieee80211_assoc_response *resp, struct ieee80211softmac_network *net);
...
static void
ieee80211softmac_assoc_adhoc(struct ieee80211softmac_device *mac, struct ieee80211softmac_network *net)
{
	unsigned long flags;
	...
	spin_lock_irqsave(&mac->lock, flags);
	ieee80211softmac_associated(mac, NULL, net);
...
ieee80211softmac_assoc_timeout(struct work_struct *work)
{
	...
	struct ieee80211softmac_network *n = NULL;
	...
	n = ieee80211softmac_get_network_by_bssid(mac, mac->associnfo.bssid);
...
ieee80211softmac_assoc_work(struct work_struct *work)
{
	...
	if (bssvalid || mac->associnfo.bssfixed) {
                found = ieee80211softmac_get_network_by_bssid_with_channel(mac, mac->associnfo.bssid, mac->associnfo.channel);
	}
	...
	mac->associnfo.bssvalid = 1;
        mac->associnfo.channel    = found->channel;
	...
	if (found->mode == IW_MODE_ADHOC) {
		ieee80211softmac_assoc_adhoc(mac, found);
				 ...
				 struct ieee80211_disassoc *disassoc)
{
	...
	dprintk(KERN_INFO PFX "got disassoc frame\n");
        ...
        /* Delete this network, so a rescan is triggered when again trying to
         * reassociate. This is necessary because of channel switches and we
         * don't want logic to find this channel again. */
        network = ieee80211softmac_get_network_by_bssid_with_channel( mac, mac->associnfo.bssid, mac->associnfo.channel );
	ieee80211softmac_disassoc( mac );  /* associnfo no longer valid */

	spin_lock_irqsave(&mac->lock, flags);
        ieee80211softmac_del_network_locked(mac, network);
        if(network->challenge != NULL)
                kfree(network->challenge);
        kfree(network);
	spin_unlock_irqrestore(&mac->lock, flags);

        /* delete all ieee802.11 networks */
        spin_lock_irqsave(&mac->ieee->lock, flags);
        list_for_each_entry_safe(net, nettmp, &mac->ieee->network_list, list) {
		struct ieee80211_network* net80211 = list_entry( &net->list, struct ieee80211_network, list);
		if( !memcmp(net80211->bssid, mac->associnfo.bssid, ETH_ALEN))
                        list_del(&net80211->list);
        }
        spin_unlock_irqrestore(&mac->ieee->lock, flags);
