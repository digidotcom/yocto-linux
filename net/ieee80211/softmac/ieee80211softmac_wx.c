			      ...
			      char *extra)
{
	...
	mutex_lock(&sm->associnfo.mutex);
        ...
        /* Doe not cancel resetting of essid if new essid is equal to the old
           essid.
           Because if using wpa_supplicant and setting the same network with
           "select x" or reassociating with "reassociate", wpa_supplicant removes
           key information and tries to reassociate. But we did not do a rescan and
           assoc_work. Now wireless system is stalled until the AP is sending a
           disconnect_event. */
