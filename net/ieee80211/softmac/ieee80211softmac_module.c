...
//	softmac->wq = create_freezeable_workqueue("softmac");
	softmac->wq = create_workqueue("softmac");
	if (!softmac->wq) {
		free_ieee80211(dev);
		return NULL;
	}
	...
	softmac->ieee->handle_disassoc = ieee80211softmac_handle_disassoc;
 	softmac->ieee->handle_beacon = ieee80211softmac_handle_beacon;
        softmac->ieee->handle_probe_response = ieee80211softmac_handle_probe_resp;
	softmac->scaninfo = NULL;

	softmac->associnfo.scan_retry = IEEE80211SOFTMAC_ASSOC_SCAN_RETRY_LIMIT;
        softmac->associnfo.channel = -1;
...
static void ieee80211softmac_start_check_rates(struct ieee80211softmac_device *mac)
{
	...
	if (mac->ieee->modulation & IEEE80211_CCK_MODULATION) {
		ri->rates[ri->count++] = IEEE80211_CCK_RATE_1MB | IEEE80211_BASIC_RATE_MASK;
		ri->rates[ri->count++] = IEEE80211_CCK_RATE_2MB | IEEE80211_BASIC_RATE_MASK;
		ri->rates[ri->count++] = IEEE80211_CCK_RATE_5MB | IEEE80211_BASIC_RATE_MASK;
		ri->rates[ri->count++] = IEEE80211_CCK_RATE_11MB | IEEE80211_BASIC_RATE_MASK;
	}
	...
	struct ieee80211_network *net)
{
	...
	softnet->mode = (net->capability & WLAN_CAPABILITY_ESS)? IW_MODE_INFRA: IW_MODE_ADHOC;
...
EXPORT_SYMBOL_GPL(ieee80211softmac_get_network_by_bssid_locked);
...
/* Get a network from the list by MAC and channel while locked */
struct ieee80211softmac_network *
...
/* Get a network from the list by BSSID and channel with locking */
struct ieee80211softmac_network *
ieee80211softmac_get_network_by_bssid_with_channel(
        struct ieee80211softmac_device *mac,
	u8 *bssid,
        u8 channel )
{
	...
	spin_lock_irqsave(&mac->lock, flags);
	softmac_net = ieee80211softmac_get_network_by_bssid_with_channel_locked(mac, bssid,channel);
	spin_unlock_irqrestore(&mac->lock, flags);
	return softmac_net;
}
...
EXPORT_SYMBOL_GPL(ieee80211softmac_get_network_by_essid_locked);
