...
ieee80211softmac_auth_resp(struct net_device *dev, struct ieee80211_auth *auth)
{
	...
	case WLAN_AUTH_OPEN:
		...
		case WLAN_STATUS_SUCCESS:
			...
			/* Send event */
			dprintkl(KERN_NOTICE PFX "Open Authentication completed with "MAC_FMT"\n", MAC_ARG(net->bssid));
		...
		default:
			...
			dprintkl(KERN_NOTICE PFX "Open Authentication with "MAC_FMT" failed, error code: %i\n",
	...
	case WLAN_AUTH_SHARED_KEY:
		...
		case IEEE80211SOFTMAC_AUTH_SHARED_CHALLENGE:
			...
			if (*data++ != MFIE_TYPE_CHALLENGE) {
				dprintkl(KERN_NOTICE PFX "Shared Key Authentication failed due to a missing challenge.\n");
				break;
			}
			/* Save the challenge */
			spin_lock_irqsave(&mac->lock, flags);
		...
		case IEEE80211SOFTMAC_AUTH_SHARED_PASS:
			...
			case WLAN_STATUS_SUCCESS:
				...
				net->authenticated = 1;
				spin_unlock_irqrestore(&mac->lock, flags);
				dprintkl(KERN_NOTICE PFX "Shared Key Authentication completed with "MAC_FMT"\n",
					MAC_ARG(net->bssid));
				ieee80211softmac_call_events(mac, IEEE80211SOFTMAC_EVENT_AUTHENTICATED, net);
				break;
			default:
				dprintkl(KERN_NOTICE PFX "Shared Key Authentication with "MAC_FMT" failed, error code: %i\n",
					MAC_ARG(net->bssid), le16_to_cpup(&auth->status));
				/* Lock and reset flags */
				spin_lock_irqsave(&mac->lock, flags);
 				net->authenticating = 0;
